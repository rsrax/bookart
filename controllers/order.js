...
exports.create = (req, res) => {
  ...
  let HelperOptions2 = {
    ...
    subject: "Your order on Bookart was successful",
    text: "Hello " + req.profile.name + ", \n\nYou have successfully made purchase of Rs. " + req.body.order.amount + " on Bookart. Please check your dashboard to track the status of your order.\n\nRegards, \nBookart"
  };
...
exports.addOrderToUserHistory = async (req, res, next) => {
  ...
  User.updateOne({ _id: req.profile._id }, { $push: { history: history } }, { new: true }, (err, data) => {
    if (err) {
      return res.status(400).json({
